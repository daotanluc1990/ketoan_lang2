...
export function MetricCard({ label, value, hint, trend, status = 'neutral', compact = false }: { label: string; value: string; hint?: string; trend?: string; status?: Status; compact?: boolean }) {
  ...
  return (
    <div className={clsx('rounded-xl bg-white shadow-soft ring-1 ring-black/5', compact ? 'p-2.5' : 'p-3')}>
      <div className="flex items-start justify-between gap-2">
        <p className="line-clamp-2 text-[11px] font-bold uppercase tracking-wide text-black/50">{label}</p>
        <StatusBadge status={status} />
      </div>
      <div className="number mt-2 text-[22px] font-extrabold leading-none text-lang-brown">{value}</div>
      {trend ? <p className={clsx('mt-1 line-clamp-1 text-[11px] font-bold leading-4', trendTone)}>{trend}</p> : null}
      {hint ? <p className="mt-0.5 line-clamp-1 text-[11px] leading-4 text-black/55">{hint}</p> : null}
    </div>
  );
}

...
export function Sidebar({ collapsed, onToggle, role }: { collapsed: boolean; onToggle: () => void; role: import('@/lib/report-types').Role }) {
  ...
  return (
    <aside className={clsx('fixed inset-y-0 left-0 z-30 hidden h-screen flex-col overflow-hidden bg-lang-redDark text-white shadow-soft transition-[width] duration-200 lg:flex', collapsed ? 'w-[72px]' : 'w-64')}>
      ...
      <div className={clsx('sidebar-foot shrink-0 border-t border-white/10 px-4 py-3 text-[11px] text-white/60', collapsed && 'hidden')}>V4.9 · RBAC server-side · Production strict</div>
    </aside>
  );
}

...
function SidebarHeader({ collapsed, onClose, onToggle, mobile = false }: { collapsed: boolean; onClose?: () => void; onToggle?: () => void; mobile?: boolean }) {
  return (
    <div className={clsx('flex h-[58px] shrink-0 items-center border-b border-lang-line bg-white', collapsed ? 'justify-center px-2' : 'justify-between px-4')}>
      <div className="flex min-w-0 items-center gap-2.5">
        <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-lang-red text-white shadow-sm">
      ...
      {!collapsed && !mobile ? (
        <button type="button" onClick={onToggle} className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-lang-muted hover:bg-lang-mist hover:text-lang-ink" aria-label="Thu gọn sidebar">
          <ChevronLeft className="h-4 w-4" />
        </button>
      ) : null}
      {mobile ? (
        <button type="button" onClick={onClose} className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-lang-muted hover:bg-lang-mist hover:text-lang-ink" aria-label="Đóng menu">
          <X className="h-4 w-4" />
        </button>
      ) : null}
    </div>
  ...
  return (
    <>
      <aside className={clsx('fixed inset-y-0 left-0 z-30 hidden h-screen flex-col overflow-hidden border-r border-lang-line bg-white text-lang-ink shadow-soft transition-[width] duration-200 lg:flex', collapsed ? 'w-[72px]' : 'w-60')}>
        ...
        {collapsed ? (
          <button type="button" onClick={onToggle} className="mx-auto mt-3 inline-flex h-9 w-9 items-center justify-center rounded-lg text-lang-muted hover:bg-lang-mist hover:text-lang-ink" aria-label="Mở rộng sidebar">
            <ChevronRight className="h-4 w-4" />
          </button>
        ) : null}
        <SidebarNav collapsed={collapsed} role={role} />
        {!collapsed ? (
          <div className="sidebar-foot shrink-0 border-t border-lang-line px-4 py-3 text-[11px] font-semibold text-lang-muted">V8.5 Saigon Ops Fresh</div>
        ) : (
          <Menu className="mx-auto mb-4 h-4 w-4 shrink-0 text-lang-muted" />
        )}
      </aside>
      ...
      <aside className={clsx('fixed inset-y-0 left-0 z-50 flex w-[min(86vw,320px)] flex-col overflow-hidden border-r border-lang-line bg-white text-lang-ink shadow-card transition-transform duration-200 ease-out lg:hidden', mobileOpen ? 'translate-x-0' : '-translate-x-full')} aria-label="Menu di động" aria-hidden={!mobileOpen}>
        ...
        <div className="shrink-0 border-t border-lang-line px-4 py-3 text-[11px] font-semibold text-lang-muted">V8.5 Saigon Ops Fresh · Mobile</div>
      </aside>
    </>
  );
}

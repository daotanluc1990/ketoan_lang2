...
const SPECS: Record<string, SubtabDashboardSpec> = {
  '/tong-quan-ke-toan': {
    ...
    focus: 'Điều hành nhanh chất lượng dữ liệu, doanh thu, tiền, công nợ, kho, thất thoát và task đỏ trước khi chốt.',
    kpiCodes: ['DQ001', 'DQ002', 'DT001', 'TM008', 'TM005', 'CN001', 'KH003', 'DM001', 'DM002', 'NV002'],
  ...
  '/nhap-lieu/upload': {
    ...
    table: { title: 'Bảng file upload', headers: ['Tên file', 'Loại dữ liệu', 'Sheet đích', 'Kỳ báo cáo', 'Chi nhánh', 'Người upload', 'Tổng dòng', 'Dòng hợp lệ', 'Dòng mới', 'Dòng trùng', 'Dòng lệch', 'Dòng lỗi', 'Trạng thái import', 'Ghi chú lỗi', 'Hành động đề xuất'] },
  ...
  '/nhap-lieu/lich-su-import': {
    ...
    displayTypes: ['KPI_CARD', 'DATA_TABLE', 'ALERT_CARD'],
    table: { title: 'Lịch sử import', headers: ['Import ID', 'Batch ID', 'Thời gian', 'Loại file', 'Sheet đích', 'Người import', 'Trạng thái', 'Dòng hợp lệ', 'Dòng lỗi', 'Trạng thái rollback', 'Link log'] },
  ...
  '/nhap-lieu/du-lieu-loi-thieu': {
    ...
    table: { title: 'Dữ liệu lỗi / thiếu', headers: ['Nguồn dữ liệu', 'Sheet đích', 'Module ảnh hưởng', 'Dòng/Cột lỗi', 'Giá trị lỗi', 'Mức độ', 'Lý do thiếu/lỗi', 'Người phụ trách', 'Deadline', 'Trạng thái', 'Hành động đề xuất'] },
  ...
  '/kho-cua-hang/ton-kho': {
    ...
    focus: 'Xem tồn lý thuyết, kiểm kê, lệch tồn và giá trị thất thoát; số bán được quy đổi sang NVL dùng theo công thức/định mức.',
    kpiCodes: ['KH001', 'KH002', 'KH005', 'KH003', 'KH004', 'TT001', 'TT002'],
    displayTypes: ['KPI_CARD', 'DATA_TABLE', 'ALERT_CARD', 'TOP_LIST'],
    table: { title: 'Bảng XNT kho cửa hàng', headers: ['Mã hàng', 'Tên hàng', 'ĐVT', 'Tồn đầu', 'Nhập BTT', 'Nhập NCC', 'NVL dùng từ số bán x định mức', 'Hủy hợp lệ', 'Tồn lý thuyết', 'Kiểm kê thực tế', 'Chênh lệch', 'Giá trị thất thoát (VND)', 'Tỷ lệ TT (%)', ...comparison, 'Trạng thái'] },
  ...
  '/kho-cua-hang/cong-thuc-dinh-muc': {
    ...
    focus: 'Kiểm định mức món bán: file bán hàng chỉ cung cấp số lượng bán, app suy ra NVL đáng lẽ dùng theo công thức chế biến.',
    kpiCodes: ['DT004', 'DM001', 'DM002', 'SX002', 'SX001'],
    displayTypes: ['KPI_CARD', 'DATA_TABLE', 'ALERT_CARD', 'TOP_LIST'],
  ...
  '/kho-bep-trung-tam/nhap-ncc': {
    ...
    displayTypes: ['KPI_CARD', 'DATA_TABLE', 'ALERT_CARD'],
    table: { title: 'Bảng nhập NCC', headers: ['Ngày', 'NCC', 'Mặt hàng', 'ĐVT', 'Số lượng nhập', 'Đơn giá', 'Thành tiền', 'Chứng từ', ...comparison, 'NCC tăng giá (%)', 'Trạng thái kiểm hàng', 'Hành động đề xuất'] },
  ...
  '/kho-bep-trung-tam/san-xuat-cong-thuc': {
    ...
    focus: 'Theo dõi mẻ sản xuất, đầu vào/đầu ra và hao hụt chế biến theo công thức được duyệt.',
    kpiCodes: ['SX002', 'SX001', 'DM001', 'DM002', 'TT001', 'TT002'],
    displayTypes: ['KPI_CARD', 'DATA_TABLE', 'ALERT_CARD', 'TREND_CHART'],
    table: { title: 'Bảng sản xuất / sơ chế BTT', headers: ['Ngày', 'Mẻ sản xuất', 'NVL đầu vào', 'ĐVT', 'Thành phẩm', 'Hao hụt chuẩn', 'Hao hụt thực tế', 'Chênh lệch SL', 'Giá trị hao hụt (VND)', 'Tỷ lệ hao hụt (%)', 'Người phụ trách', ...comparison, 'Trạng thái', 'Hành động đề xuất'] },
  ...
  '/kho-bep-trung-tam/ton-kho-hao-hut': {
    ...
    kpiCodes: ['KH001', 'KH002', 'KH005', 'KH003', 'KH004', 'TT001', 'TT002'],
    displayTypes: ['KPI_CARD', 'DATA_TABLE', 'ALERT_CARD', 'TOP_LIST'],
    table: { title: 'Bảng XNT BTT', headers: ['Ngày', 'Kho', 'Mã hàng', 'Tên hàng', 'ĐVT', 'Tồn đầu', 'Nhập NCC', 'Sản xuất/sơ chế', 'Xuất cửa hàng', 'Cửa hàng nhận', 'Xác nhận CH', 'Hủy BTT', 'Tồn lý thuyết', 'Tồn thực tế', 'Chênh lệch', 'Giá trị thất thoát (VND)', 'Tỷ lệ TT (%)', ...comparison, 'Trạng thái', 'Người xử lý', 'Hành động đề xuất'] },
  ...
  '/tai-chinh/tong-quan': {
    ...
    kpiCodes: ['TM008', 'TM001', 'CP001', 'CP006', 'CP007', 'CP002', 'CN001', 'CN002'],
    displayTypes: ['KPI_CARD', 'DATA_TABLE', 'ALERT_CARD'],
    table: { title: 'Bảng tổng quan tài chính', headers: ['Ngày', 'Loại nghiệp vụ', 'Nhóm khoản mục', 'Đơn vị', 'Số tiền', 'Dự toán', 'Thực tế', 'Chứng từ', ...comparison, 'Trạng thái', 'Hành động đề xuất'] },
  ...
  '/tai-chinh/dong-tien': {
    ...
    kpiCodes: ['TM008', 'TM001', 'CP001', 'CP006', 'CP007', 'CP002', 'CN001'],
    displayTypes: ['KPI_CARD', 'DATA_TABLE', 'TREND_CHART', 'BAR_CHART'],
    table: { title: 'Bảng dòng tiền', headers: ['Ngày', 'Nhóm thu/chi', 'Đơn vị', 'Số tiền', 'Tỷ trọng', 'Dòng tiền hiện tại', 'Dòng tiền dự kiến', 'Mức thiếu tiền dự kiến', ...comparison, 'Xu hướng', 'Trạng thái', 'Hành động đề xuất'] },
  ...
  '/tai-chinh/can-doi': {
    ...
    displayTypes: ['KPI_CARD', 'DATA_TABLE', 'ALERT_CARD'],
    table: { title: 'Bảng cân đối rút gọn', headers: ['Nhóm tài sản/công nợ', 'Đơn vị', 'Giá trị', 'Công nợ đến hạn', 'Phải thu', 'Phải trả', 'Ghi chú', ...comparison, 'Trạng thái', 'Hành động đề xuất'] },
  ...
  '/tai-chinh/du-toan': {
    ...
    kpiCodes: ['DT001', 'DT008', 'DT009', 'DT010', 'DT011', 'CP001', 'CN001', 'LN003'],
    displayTypes: ['KPI_CARD', 'DATA_TABLE', 'TREND_CHART'],
    table: { title: 'Bảng dự toán', headers: ['Kỳ dự toán', 'Đơn vị', 'Doanh thu dự kiến', 'Chi dự kiến', 'Công nợ đến hạn', 'Số dư đầu kỳ', 'Số dư cuối kỳ dự kiến', 'Thực tế', 'Dự toán', 'Chênh lệch thực tế/dự toán', 'Tỷ lệ chênh lệch thực tế/dự toán (%)', 'Cảnh báo thiếu tiền', 'Hành động đề xuất'] },
  ...
  '/bao-cao-quan-tri/tuan': {
    ...
    kpiCodes: ['BC001', 'BC002', 'DQ001', 'LN003', 'LN004', 'LN001', 'LN002', 'DM001', 'DM002', 'CN001'],
  ...
  '/bao-cao-quan-tri/thang': {
    ...
    kpiCodes: ['BC001', 'BC002', 'DQ001', 'DT001', 'LN003', 'LN004', 'LN001', 'LN002', 'DM001', 'DM002'],
    displayTypes: ['KPI_CARD', 'DATA_TABLE', 'ALERT_CARD', 'TASK_LIST'],

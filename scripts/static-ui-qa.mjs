...
const cockpit = read('src/components/dashboard/CeoCockpitPage.tsx');
for (const text of ['CEO Cockpit', 'Việc CEO cần xử lý ngay', 'Độ sẵn sàng dữ liệu']) assert(cockpit.includes(text), `CEO cockpit missing ${text}`);
assert(cockpit.includes('2xl:grid-cols-8'), 'CEO cockpit KPI strip must be dense on large desktop');
...
console.log('Static UI QA passed: 9 tabs, compact headers, dense KPI strips, internal-scroll tables, compact import, CEO cockpit, accountant workflow, and reduced explanation blocks.');

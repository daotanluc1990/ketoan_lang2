  ...
  try {
    if (body.batch?.files) {
      ...
      for (const preview of previews) {
        results.push(await confirmImport(preview, body.actor ?? rbac.context.actor, { allowPartial: body.allowPartial === true }));
      }
    ...
    const result = await confirmImport(preview, body.actor ?? rbac.context.actor, { allowPartial: body.allowPartial === true });
